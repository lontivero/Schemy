...
class Program
{
    ...
    static void Main(string[] args)
    {
        Interpreter.CreateSymbolTableDelegate extension = _ => new Dictionary<Symbol, object>()
        {
            ...
            { Symbol.FromString("truncate-string"), NativeProcedure.Create<int, Function>(len => input => ((string)input)[..len]) },
        };

        var interpreter = new Interpreter([extension]);
